...
// 内网检测结果缓存（每次页面加载只查一次后端）
let _isLanCached: boolean | null = null

// 通过后端 API 检测客户端是否来自内网
async function checkIsLanFromServer(): Promise<boolean> {
  if (_isLanCached !== null) return _isLanCached
  ...
  try {
    ...
    // noop, continue with browser-side probe fallback
  }

  const pingUrl = getLanPingUrl()
  if (pingUrl) {
    _isLanCached = await testLanPingUrl(pingUrl)
    return _isLanCached
  }
...
async function handleItemClick(itemGroupIndex: number, item: Panel.ItemInfo) {
  ...
  // 关键规则：点击打开时必须优先尊重“当前手动选择的网络模式”
  // - LAN 模式：优先内网地址，没有就回退公网地址
  // - WAN 模式：始终使用公网地址
  // 不再在点击时重新探测并覆盖用户手动切换结果。
  const jumpUrl = panelState.networkMode === PanelStateNetworkModeEnum.lan && isValidUrl(item.lanUrl)
    ? lanUrl
    : publicUrl
...
function filterItemsByNetworkMode() {
  ...
  if (panelState.networkMode === PanelStateNetworkModeEnum.wan) {
    ...
    // 过滤掉没有项目的组（仅过滤 lanOnly 项目后真正为空的组）
    filterItems.value = filteredGroups
  } else {
    // 私密模式下显示所有项目
...
function handleBookmarkIconError(event: Event) {
  ...
  if (target.src === DEFAULT_BOOKMARK_ICON)
    return
...
function matchesItemSearchKeyword(item: Panel.ItemInfo, lowerCaseKeyword: string) {
  ...
  if (trimmedKeyword !== '' && panelState.panelConfig.searchBoxSearchIcon) {
    ...
    for (let i = 0; i < items.value.length; i++) {
      const element = items.value[i].items?.filter((item: Panel.ItemInfo) => {
        ...
        // 然后应用搜索关键词过滤
        return matchesItemSearchKeyword(item, lowerCaseKeyword)
      })
...
isFolder ? h('svg', {
        ...
        return h('img', {
          src: getBookmarkDisplayIcon(option.bookmark?.iconJson),
